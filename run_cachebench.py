...
def updateConfig(config, dsaDevs, threads, batch):
    pwd = os.path.dirname(os.path.abspath(sys.argv[0]))
    dsaEnabled = str(True if dsaDevs > 0 else False).lower()
    for i in range(len(gDsaDevList)):
        if i < dsaDevs:
            cmd = [os.path.join(pwd, "accel-config.sh"), gDsaDevList[i], "yes"]
            subprocess.run(cmd)
        else:
            cmd = [os.path.join(pwd, "accel-config.sh"), gDsaDevList[i], "no"]
            subprocess.run(cmd)
    ...
    with open(absPath, "r") as f:
        ...
        data["cache_config"]["evictorThreads"]   = threads
        data["cache_config"]["maxEvictionBatch"] = batch
...
def runTest(config, outputPath):
    ...
    perfCmd = ["perf", "stat", "-e",
               "dsa0/event=0x1,event_category=0x0/,\
                ...
                dsa8/event=0x1,event_category=0x0/".replace(' \\', '').replace(' ', '')]
...
def main():
    args = sys.argv[1:]
    if len(args) != 5:
        print("Invalid args. Required : config-file, dsa-device-count, background-threads, batch-size, output-path")
        exit()
    ...
    updateConfig(config, dsaDevs, threads, batch)
